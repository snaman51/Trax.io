  ...
  <v-app id="inspire">
    ...
    <v-card height="100%">
      ...
      >
        ...
        <v-spacer></v-spacer>
        <v-btn class="ma-2" outlined color="white" to="/custCreate">
          Add Customer
        </v-btn>
      </v-toolbar>
      <v-toolbar
      ...
      >
        ...
        <v-spacer></v-spacer>
        <v-btn class="ma-2" outlined color="white" to="/sellerCreate">
          Add Seller
        </v-btn>
      </v-toolbar>
      <v-toolbar
      ...
      >
        ...
        <v-spacer></v-spacer>
        <v-btn class="ma-2" outlined color="white" to="/courierCreate">
          Add Courier
        </v-btn>
      </v-toolbar>
      <v-card-text>
        ...
        >
          ...
          <template slot="courierArr" scope="props">
            <td v-for="header in courierheaders" :key="header">
              {{ props.item[header.value] }}
            </td>
          </template>
        </v-data-table>
...
export default {
  ...
  computed: {
    pages() {
      return this.footerProps.rowsPerPage
  ...
  methods: {
          ...
          .then((snapshot) => {
            snapshot.forEach((doc) => {
              if (this.toggle == "customer") {
                ...
                console.log("--------------------", this.toggle, courier);
                courier.id = doc.id;
    ...
    editCourier(value) {
      this.$router.push({ path: `/courierDetails/${value.id}` });
    },
  },
};
</script>